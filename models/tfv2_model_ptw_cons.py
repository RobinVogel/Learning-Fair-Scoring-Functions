...
from models.tfv2_model_auc_cons import (AUCCons, gen_batch, incomplete_auc,
...
class PtwCons(AUCCons):
    ...
    def define_cost(self, l2_reg):
        ...
        self.ph_c = tf.compat.v1.placeholder(tf.float32, [self.n_c], name="c")
        self.ph_thre = tf.compat.v1.placeholder(tf.float32, [self.n_c], name="thre")
        ...
        for y_val in self.mon_pts.keys():
            ...
            for mon_pt in self.mon_pts[y_val]:
                ...
                n_z_1 = tf.reduce_sum(input_tensor=tf.cast(filt_1, tf.int32))
                no_z_1 = tf.equal(n_z_1, 0)
                no_z_0 = tf.equal(tf.shape(input=filt_1)[0] - n_z_1, 0)
                ...
                rel_tpr = tf.cond(pred=no_z_1, true_fn=lambda: 0.,
                                  false_fn=lambda: tf.reduce_mean(input_tensor=r_p_pred[filt_1]))
                rel_fpr = tf.cond(pred=no_z_0, true_fn=lambda: 0.,
                                  false_fn=lambda: tf.reduce_mean(input_tensor=r_p_pred[filt_0]))

                # Monitored values:
                tpr = tf.cond(pred=no_z_1, true_fn=lambda: 0.,
                              false_fn=lambda: tf.cast(tf.greater(biased_sc[filt_1], 0),
                                              tf.float32))
                fpr = tf.cond(pred=no_z_0, true_fn=lambda: 0.,
                              false_fn=lambda: tf.cast(tf.greater(biased_sc[filt_0], 0),
                                              tf.float32))

                mean_tpr = tf.reduce_mean(input_tensor=tpr)
                mean_fpr = tf.reduce_mean(input_tensor=fpr)
