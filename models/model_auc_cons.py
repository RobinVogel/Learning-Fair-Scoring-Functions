...
def full_auc(sc, y, rel):
    y = tf.reshape(y, (-1, 1))
    delta_y = y - tf.transpose(y)
    n_diff = tf.reduce_sum(tf.cast(tf.not_equal(delta_y, 0), tf.int32))

    sc_loc = sc

    def compute_value():
        sc = tf.reshape(sc_loc, (-1, 1))
        delta_sc = sc - tf.transpose(sc)
        filt = tf.greater(delta_y, 0.)
        # We only select relevant instances to multiply:
        prod = delta_sc[filt]*delta_y[filt]
        rel_auc = tf.reduce_mean(rel(prod))
        auc = (tf.reduce_mean(tf.cast(tf.greater(0., prod), tf.float32)) +
               0.5*tf.reduce_mean(tf.cast(tf.equal(prod, 0.), tf.float32)))
        return rel_auc, auc

    cond = tf.equal(n_diff, 0)
    return tf.cond(cond, lambda: (0, 0), compute_value)
...
def incomplete_auc(sc, y, rel, B=1000):
    ...
    num_n = tf.shape(n_y)[0]
    num_p = tf.shape(p_y)[0]
...
class AUCCons(GeneralModel):
    ...
    def fit(self, data_train, model_folder=None):
        ...
        if self.optimizer == "momentum":
            optim_step = tf.train.MomentumOptimizer(learning_rate=self.lr,
                                                    momentum=self.momentum)
        elif self.optimizer == "adam":
            optim_step = tf.train.AdamOptimizer(learning_rate=self.lr,
                                                beta1=self.momentum)
    ...
    def define_model(self):
        ...
        self.ph_x = tf.placeholder(tf.float32,
                                   ...
                                   name="features")
        self.ph_y = tf.placeholder(tf.float32, [None], name="y")
        self.ph_z = tf.placeholder(tf.float32, [None], name="z")
    ...
    def define_main_obj(self):
        if self.use_inc_auc_train:
            self.ph_incomplete = tf.placeholder(tf.int32, (), name="B")
    ...
    def define_cost(self, l2_reg):
        ...
        self.ph_c = tf.placeholder(tf.float32, (), name="c")
    ...
    def define_net(self, ph_x):
        ...
        s_out = tf.add(tf.matmul(fc1, self.weights['w_out']),
                       self.biases['b_out'])
        s_out = tf.layers.batch_normalization(s_out, trainable=False,
